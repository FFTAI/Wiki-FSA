...
template < typename OutputIt > auto write_encoded_tm_str( OutputIt out, string_view in, const std::locale& loc ) -> OutputIt {
    if ( detail::is_utf8() && loc != get_classic_locale() ) {
        ...
        unicode_to_utf8< code_unit, basic_memory_buffer< char, unit_t::max_size * 4 > > u;
...
// DEPRECATED!
template < typename Char > FMT_CONSTEXPR auto parse_align( const Char* begin, const Char* end, format_specs< Char >& specs ) -> const Char* {
...
template < typename Char, typename OutputIt, typename Duration > void write_fractional_seconds( OutputIt& out, Duration d, int precision = -1 ) {
    ...
    const auto subseconds =
        std::chrono::treat_as_floating_point< typename subsecond_precision::rep >::value ? fractional.count() : std::chrono::duration_cast< subsecond_precision >( fractional ).count();
...
template < typename Duration > void write_floating_seconds( memory_buffer& buf, Duration duration, int num_fractional_digits = -1 ) {
    ...
    format_to( std::back_inserter( buf ), FMT_STRING( "{:.{}f}" ),
               std::fmod( val * static_cast< rep >( Duration::period::num ) / static_cast< rep >( Duration::period::den ), static_cast< rep >( 60 ) ), num_fractional_digits );
}
...
template < typename Char, typename Rep, typename OutputIt, FMT_ENABLE_IF( std::is_integral< Rep >::value ) > OutputIt format_duration_value( OutputIt out, Rep val, int ) {
    return write< Char >( out, val );
}

template < typename Char, typename Rep, typename OutputIt, FMT_ENABLE_IF( std::is_floating_point< Rep >::value ) > OutputIt format_duration_value( OutputIt out, Rep val, int precision ) {
...
private:
    ...
    FMT_CONSTEXPR parse_range do_parse( basic_format_parse_context< Char >& ctx ) {
        ...
        if ( *begin == '.' ) {
            ...
            begin                          = detail::parse_precision( begin, end, precision, precision_ref, ctx );
        }
        if ( begin != end && *begin == 'L' ) {
            ++begin;
            localized = true;
...
template < typename Char, typename Duration > struct formatter< std::chrono::time_point< std::chrono::system_clock, Duration >, Char > : formatter< std::tm, Char > {
    ...
    template < typename FormatContext > auto format( std::chrono::time_point< std::chrono::system_clock, Duration > val, FormatContext& ctx ) const -> decltype( ctx.out() ) {
...
#if FMT_USE_LOCAL_TIME
template < typename Char, typename Duration > struct formatter< std::chrono::local_time< Duration >, Char > : formatter< std::tm, Char > {
    FMT_CONSTEXPR formatter() {
        this->format_str = detail::string_literal< Char, '%', 'F', ' ', '%', 'T' >{};
    }

    template < typename FormatContext > auto format( std::chrono::local_time< Duration > val, FormatContext& ctx ) const -> decltype( ctx.out() ) {
        using period = typename Duration::period;
        if ( period::num != 1 || period::den != 1 || std::is_floating_point< typename Duration::rep >::value ) {
            const auto epoch   = val.time_since_epoch();
...
struct formatter< std::chrono::time_point< std::chrono::utc_clock, Duration >, Char > : formatter< std::chrono::time_point< std::chrono::system_clock, Duration >, Char > {
    template < typename FormatContext > auto format( std::chrono::time_point< std::chrono::utc_clock, Duration > val, FormatContext& ctx ) const -> decltype( ctx.out() ) {
...
protected:
    ...
    FMT_CONSTEXPR auto do_parse( basic_format_parse_context< Char >& ctx ) -> decltype( ctx.begin() ) {
        ...
        if ( end != begin )
            format_str = { begin, detail::to_unsigned( end - begin ) };
        return end;
    }

    template < typename FormatContext, typename Duration > auto do_format( const std::tm& tm, FormatContext& ctx, const Duration* subsecs ) const -> decltype( ctx.out() ) {
        auto                        specs_copy = specs;
        basic_memory_buffer< Char > buf;
        auto                        out = std::back_inserter( buf );
        detail::handle_dynamic_spec< detail::width_checker >( specs_copy.width, width_ref, ctx );
