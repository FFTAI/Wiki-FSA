...
#define EIGEN_VERSION_AT_LEAST( x, y, z ) ( EIGEN_WORLD_VERSION > x || ( EIGEN_WORLD_VERSION >= x && ( EIGEN_MAJOR_VERSION > y || ( EIGEN_MAJOR_VERSION >= y && EIGEN_MINOR_VERSION >= z ) ) ) )
...
#if EIGEN_MAX_CPP_VER >= 11 && ( __has_feature( cxx_rvalue_references ) || ( EIGEN_COMP_CXXVER >= 11 ) || ( EIGEN_COMP_MSVC >= 1600 ) )
...
#define EIGEN_HAS_STD_INVOKE_RESULT 0
    ...
    && ( __has_feature( cxx_alignas ) || EIGEN_HAS_CXX14 || ( EIGEN_COMP_MSVC >= 1800 ) || ( EIGEN_GNUC_AT_LEAST( 4, 8 ) ) || ( EIGEN_COMP_CLANG >= 305 ) || ( EIGEN_COMP_ICC >= 1500 ) \
...
#if EIGEN_MAX_CPP_VER >= 11 && ( EIGEN_HAS_CXX11 || EIGEN_COMP_MSVC >= 1700 ) && ( ( !EIGEN_COMP_GNUC_STRICT ) || EIGEN_GNUC_AT_LEAST( 5, 1 ) ) \
...
#define EIGEN_HAS_CONSTEXPR 1
    ...
    && ( __has_feature( cxx_relaxed_constexpr ) || ( EIGEN_COMP_CXXVER >= 14 ) || ( EIGEN_GNUC_AT_LEAST( 4, 8 ) && ( EIGEN_COMP_CXXVER >= 11 ) ) \
...
// Let's be conservative and enable the default C++11 implementation only if we are sure it exists
    ...
    && ( ( EIGEN_COMP_CXXVER > 11 )                                                                                                                             \
         || ( EIGEN_COMP_CXXVER == 11 ) && ( EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG || EIGEN_COMP_MSVC || EIGEN_COMP_ICC ) && ( EIGEN_ARCH_i386_OR_x86_64 ) \
...
#if EIGEN_MAX_CPP_VER >= 11 && ( ( EIGEN_COMP_CXXVER > 11 ) || ( ( EIGEN_COMP_CXXVER == 11 ) && ( EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG || EIGEN_COMP_MSVC || EIGEN_COMP_ICC >= 1400 ) ) )
...
#if EIGEN_MAX_CPP_VER >= 11 && ( __has_feature( cxx_atomic ) || ( EIGEN_COMP_CXXVER > 11 ) || ( ( EIGEN_COMP_CXXVER == 11 ) && ( EIGEN_COMP_MSVC == 0 || EIGEN_COMP_MSVC >= 1700 ) ) )
...
#if EIGEN_MAX_CPP_VER >= 17 && EIGEN_COMP_CXXVER >= 17                                                                                                \
    && ( ( EIGEN_COMP_MSVC >= 1912 ) || ( EIGEN_GNUC_AT_LEAST( 7, 0 ) ) || ( ( !defined( __apple_build_version__ ) ) && ( EIGEN_COMP_CLANG >= 500 ) ) \
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE( Derived )                                                                                                                                                     \
    ...
    typedef typename Base::CoeffReturnType CoeffReturnType; /*!< \brief The return type for coefficient access. \details Depending on whether the object allows direct coefficient access (e.g. for a \
                                                               MatrixXd), this type is either 'const Scalar&' or simply 'Scalar' for objects that do not allow direct coefficient access. */          \
...
#define EIGEN_CWISE_BINARY_RETURN_TYPE( LHS, RHS, OPNAME ) \
    CwiseBinaryOp< EIGEN_CAT( EIGEN_CAT( internal::scalar_, OPNAME ), _op ) < typename internal::traits< LHS >::Scalar, typename internal::traits< RHS >::Scalar >, const LHS, const RHS >
...
#define EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE( SCALAR, EXPR, OPNAME )                                                                                                                                 \
    CwiseBinaryOp< EIGEN_CAT( EIGEN_CAT( internal::scalar_, OPNAME ), _op ) < SCALAR, typename internal::traits< EXPR >::Scalar >, const typename internal::plain_constant_type< EXPR, SCALAR >::type, \
        const EXPR >
...
#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHERIGHT( METHOD, OPNAME )                                                                                                                                     \
    template < typename T >                                                                                                                                                                          \
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE( const EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(                                                                \
        Derived, typename internal::promote_scalar_arg< Scalar EIGEN_COMMA T EIGEN_COMMA EIGEN_SCALAR_BINARY_SUPPORTED( OPNAME, Scalar, T ) >::type, OPNAME ) )( METHOD )( const T& scalar ) const { \
        typedef typename internal::promote_scalar_arg< Scalar, T, EIGEN_SCALAR_BINARY_SUPPORTED( OPNAME, Scalar, T ) >::type PromotedT;                                                              \
        return EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE( Derived, PromotedT, OPNAME )(                                                                                                                 \
            derived(), typename internal::plain_constant_type< Derived, PromotedT >::type( derived().rows(), derived().cols(), internal::scalar_constant_op< PromotedT >( scalar ) ) );              \
    }

#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHELEFT( METHOD, OPNAME )                                                                                                                                    \
    template < typename T >                                                                                                                                                                        \
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE friend EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE(                                                                                                     \
        const EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE( typename internal::promote_scalar_arg< Scalar EIGEN_COMMA T EIGEN_COMMA EIGEN_SCALAR_BINARY_SUPPORTED( OPNAME, T, Scalar ) >::type, Derived, \
