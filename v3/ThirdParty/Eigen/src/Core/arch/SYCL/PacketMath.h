...
#define SYCL_PLOADT_RO( address_space_target )                                                                                                                                               \
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE packet_type ploadt_ro(                                                                                                                             \
        typename cl::sycl::multi_ptr< const typename unpacket_traits< packet_type >::type, cl::sycl::access::address_space::address_space_target >::pointer_t from ) {                       \
...
#define SYCL_PLOAD( Alignment, AlignedType )                                                                                                         \
    template < typename packet_type >                                                                                                                \
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE packet_type pload##AlignedType(                                                                            \
        const Eigen::TensorSycl::internal::RangeAccess< cl::sycl::access::mode::read_write, typename unpacket_traits< packet_type >::type > from ) { \
        return ploadt_ro< packet_type, Alignment >( from );                                                                                          \
    }
...
#undef SYCL_PLOADT
    ...
    ploadt( const Eigen::TensorSycl::internal::RangeAccess< cl::sycl::access::mode::read_write, typename unpacket_traits< packet_type >::type >& from ) {
...
#define SYCL_PLOADT_RO_SPECIAL( packet_type, Alignment )                                                                                                             \
    template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE packet_type ploadt_ro< packet_type, Alignment >( const typename unpacket_traits< packet_type >::type* from ) { \
        typedef typename unpacket_traits< packet_type >::type scalar;                                                                                                \
...
#define SYCL_PLOAD_SPECIAL( packet_type, alignment_type )                                                                                              \
    template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE packet_type pload##alignment_type( const typename unpacket_traits< packet_type >::type* from ) { \
        typedef typename unpacket_traits< packet_type >::type scalar;                                                                                  \
        auto                                                  res = packet_type( static_cast< scalar >( 0 ) );                                         \
        res.template load< cl::sycl::access::address_space::private_space >( 0, const_cast< scalar* >( from ) );                                       \
        return res;                                                                                                                                    \
...
#define SYCL_PSTORE( scalar, packet_type, address_space_target, alignment )                                                                                                    \
    template <>                                                                                                                                                                \
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void pstore##alignment( typename cl::sycl::multi_ptr< scalar, cl::sycl::access::address_space::address_space_target >::pointer_t to, \
...
#define SYCL_PSTORE_T( address_space_target )                                                                                                                                                    \
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void pstoret( typename cl::sycl::multi_ptr< scalar, cl::sycl::access::address_space::address_space_target >::pointer_t to, const packet_type& from ) { \
        if ( Alignment )                                                                                                                                                                         \
            pstore( to, from );                                                                                                                                                                  \
        else                                                                                                                                                                                     \
            pstoreu( to, from );                                                                                                                                                                 \
...
#define SYCL_PSET1( packet_type )                                                                                                                     \
    template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE packet_type pset1< packet_type >( const typename unpacket_traits< packet_type >::type& from ) { \
...
#undef SYCL_PSET1
    ...
    template < typename packet_type > struct get_base_packet {
        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE packet_type get_ploaddup( sycl_multi_pointer ) {}

        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE packet_type get_pgather( sycl_multi_pointer, Index ) {}
    };

    template <> struct get_base_packet< cl::sycl::cl_float4 > {
        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE cl::sycl::cl_float4 get_ploaddup( sycl_multi_pointer from ) {
            return cl::sycl::cl_float4( from[ 0 ], from[ 0 ], from[ 1 ], from[ 1 ] );
        }
        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE cl::sycl::cl_float4 get_pgather( sycl_multi_pointer from, Index stride ) {
            return cl::sycl::cl_float4( from[ 0 * stride ], from[ 1 * stride ], from[ 2 * stride ], from[ 3 * stride ] );
        }

        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void set_pscatter( sycl_multi_pointer to, const cl::sycl::cl_float4& from, Index stride ) {
            auto tmp            = stride;
            to[ 0 ]             = from.x();
            to[ tmp ]           = from.y();
    ...
    template <> struct get_base_packet< cl::sycl::cl_double2 > {
        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE cl::sycl::cl_double2 get_ploaddup( const sycl_multi_pointer from ) {
            return cl::sycl::cl_double2( from[ 0 ], from[ 0 ] );
        }

        template < typename sycl_multi_pointer, typename Index > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE cl::sycl::cl_double2 get_pgather( const sycl_multi_pointer from, Index stride ) {
            return cl::sycl::cl_double2( from[ 0 * stride ], from[ 1 * stride ] );
        }

        template < typename sycl_multi_pointer > static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void set_pscatter( sycl_multi_pointer to, const cl::sycl::cl_double2& from, Index stride ) {
            to[ 0 ]      = from.x();
            to[ stride ] = from.y();
        }
...
#define SYCL_PLOAD_DUP_SPECILIZE( packet_type )                                                                                                          \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE packet_type ploaddup< packet_type >( const typename unpacket_traits< packet_type >::type* from ) { \
        return get_base_packet< packet_type >::get_ploaddup( from );                                                                                     \
    }
...
#define SYCL_PGATHER( address_space_target )                                                                                                                                         \
    ...
    EIGEN_DEVICE_FUNC inline packet_type pgather(                                                                                                                                    \
        typename cl::sycl::multi_ptr< const typename unpacket_traits< packet_type >::type, cl::sycl::access::address_space::address_space_target >::pointer_t from, Index stride ) { \
...
#define SYCL_PGATHER_SPECILIZE( scalar, packet_type )                                                                                                                         \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE packet_type pgather< scalar, packet_type >( const typename unpacket_traits< packet_type >::type* from, Index stride ) { \
        return get_base_packet< packet_type >::get_pgather( from, stride );                                                                                                   \
    }
...
#define SYCL_PSCATTER( address_space_target )                                                                                                                                                   \
    template < typename Scalar, typename packet_type >                                                                                                                                          \
    EIGEN_DEVICE_FUNC inline void pscatter( typename cl::sycl::multi_ptr< typename unpacket_traits< packet_type >::type, cl::sycl::access::address_space::address_space_target >::pointer_t to, \
...
#define SYCL_PSCATTER_SPECILIZE( scalar, packet_type )                                                                                                                                    \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void pscatter< scalar, packet_type >( typename unpacket_traits< packet_type >::type * to, const packet_type& from, Index stride ) { \
        get_base_packet< packet_type >::set_pscatter( to, from, stride );                                                                                                                 \
    }
...
#define SYCL_PMAD( packet_type )                                                                                                              \
    template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE packet_type pmadd( const packet_type& a, const packet_type& b, const packet_type& c ) { \
...
#undef SYCL_PMAD
    ...
    template < typename Packet > EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Packet sycl_pcmp_le( const Packet& a, const Packet& b ) {
        return ( ( a <= b ).template convert< typename unpacket_traits< Packet >::type, cl::sycl::rounding_mode::automatic >() );
    }

    template < typename Packet > EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Packet sycl_pcmp_lt( const Packet& a, const Packet& b ) {
        return ( ( a < b ).template convert< typename unpacket_traits< Packet >::type, cl::sycl::rounding_mode::automatic >() );
    }

    template < typename Packet > EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Packet sycl_pcmp_eq( const Packet& a, const Packet& b ) {
        return ( ( a == b ).template convert< typename unpacket_traits< Packet >::type, cl::sycl::rounding_mode::automatic >() );
    }

#define SYCL_PCMP( OP, TYPE )                                                                                  \
    template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE TYPE pcmp_##OP< TYPE >( const TYPE& a, const TYPE& b ) { \
...
#undef SYCL_PCMP
    ...
    template < typename PacketIn > EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE typename convert_to_integer< typename unpacket_traits< PacketIn >::type >::packet_type vector_as_int( const PacketIn& p ) {
        return ( p.template convert< typename convert_to_integer< typename unpacket_traits< PacketIn >::type >::type, cl::sycl::rounding_mode::automatic >() );
    }

    template < typename packetOut, typename PacketIn > EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE packetOut convert_vector( const PacketIn& p ) {
        return ( p.template convert< typename unpacket_traits< packetOut >::type, cl::sycl::rounding_mode::automatic >() );
    }

#define SYCL_PAND( TYPE )                                                                                 \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE pand< TYPE >( const TYPE& a, const TYPE& b ) { \
...
#define SYCL_POR( TYPE )                                                                                 \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE por< TYPE >( const TYPE& a, const TYPE& b ) { \
        return convert_vector< TYPE >( vector_as_int( a ) | vector_as_int( b ) );                        \
    }
...
#define SYCL_PXOR( TYPE )                                                                                 \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE pxor< TYPE >( const TYPE& a, const TYPE& b ) { \
        return convert_vector< TYPE >( vector_as_int( a ) ^ vector_as_int( b ) );                         \
    }
...
#define SYCL_PANDNOT( TYPE )                                                                                 \
    template <> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE pandnot< TYPE >( const TYPE& a, const TYPE& b ) { \
        return convert_vector< TYPE >( vector_as_int( a ) & ( ~vector_as_int( b ) ) );                       \
    }
...
#undef SYCL_PANDNOT
    ...
    template <>
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE cl::sycl::cl_float4 pblend( const Selector< unpacket_traits< cl::sycl::cl_float4 >::size >& ifPacket, const cl::sycl::cl_float4& thenPacket,
                                                                      const cl::sycl::cl_float4& elsePacket ) {
        cl::sycl::cl_int4 condition( ifPacket.select[ 0 ] ? 0 : -1, ifPacket.select[ 1 ] ? 0 : -1, ifPacket.select[ 2 ] ? 0 : -1, ifPacket.select[ 3 ] ? 0 : -1 );
        return cl::sycl::select( thenPacket, elsePacket, condition );
    }

    template <>
    inline cl::sycl::cl_double2 pblend( const Selector< unpacket_traits< cl::sycl::cl_double2 >::size >& ifPacket, const cl::sycl::cl_double2& thenPacket, const cl::sycl::cl_double2& elsePacket ) {
...
#define SYCL_PSTORE( alignment )                                                                                                                                             \
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void pstore##alignment(                                                                                                            \
        const Eigen::TensorSycl::internal::RangeAccess< cl::sycl::access::mode::read_write, typename unpacket_traits< packet_type >::type >& to, const packet_type& from ) { \
...
#undef SYCL_PSTORE
    ...
    template < typename scalar, typename packet_type, int Alignment >
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void pstoret( Eigen::TensorSycl::internal::RangeAccess< cl::sycl::access::mode::read_write, typename unpacket_traits< packet_type >::type > to,
                                                        const packet_type&                                                                                                            from ) {
