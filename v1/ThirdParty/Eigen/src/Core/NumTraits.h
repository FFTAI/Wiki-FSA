...
namespace internal {
    ...
    template < typename T, bool use_numeric_limits = std::numeric_limits< T >::is_specialized, bool is_integer = NumTraits< T >::IsInteger > struct default_digits10_impl {
    ...
    template < typename T, bool use_numeric_limits = std::numeric_limits< T >::is_specialized, bool is_integer = NumTraits< T >::IsInteger > struct default_digits_impl {
        EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR static int run() {
...
namespace numext {
    ...
    template < typename Tgt, typename Src > EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Tgt bit_cast( const Src& src ) {
#if EIGEN_HAS_TYPE_TRAITS
        // The behaviour of memcpy is not specified for non-trivially copyable types
        EIGEN_STATIC_ASSERT( std::is_trivially_copyable< Src >::value, THIS_TYPE_IS_NOT_SUPPORTED );
        EIGEN_STATIC_ASSERT( std::is_trivially_copyable< Tgt >::value && std::is_default_constructible< Tgt >::value, THIS_TYPE_IS_NOT_SUPPORTED );
#endif
...
template < typename T > struct GenericNumTraits {
    ...
    typedef T                                                                                                                             Real;
    typedef typename internal::conditional< IsInteger, typename internal::conditional< sizeof( T ) <= 2, float, double >::type, T >::type NonInteger;
    typedef T                                                                                                                             Nested;
    typedef T                                                                                                                             Literal;
...
template < typename Scalar, int Rows, int Cols, int Options, int MaxRows, int MaxCols > struct NumTraits< Array< Scalar, Rows, Cols, Options, MaxRows, MaxCols > > {
