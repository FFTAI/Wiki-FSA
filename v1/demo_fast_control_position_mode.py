...
def main():
    ...
    if server_ip_list:
        ...
        for i in range(len(server_ip_list)):
            fi_fsa.fast_set_position_control(server_ip_list[i], 0.0)
        ...
        for i in range(len(server_ip_list)):
            fi_fsa.fast_set_mode_of_operation(server_ip_list[i], fi_fsa.FSAModeOfOperation.POSITION_CONTROL)
        ...
        for t in range(0, count_max):
            for i in range(len(server_ip_list)):
                ...
                fi_fsa.fast_set_position_control(server_ip_list[i], set_position)
            time.sleep(0.01)
