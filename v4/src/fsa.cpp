...
int FSA::send_msg( char* ip, int port, const char* msg ) {
    if ( !this->is_init ) {
        this->init_network();
        Logger::get_instance()->print_trace( "init fsa\n" );
        this->is_init = 1;
    ...
    memcpy( this->send_buff, msg, sizeof( this->send_buff ) );
...
// int FSA::control_current_mode(char* server_ip, char *define_msg_sendto, char
// *client_recv_msg);
...
int Actuator::broadcast( char* ip_ser, int& ser_num ) {
    ...
    memcpy( ip_ser, fsa->server_ip, sizeof( fsa->server_ip ) );
...
/// @brief 获取通信配置
/// @param ip
/// @param config_buffer rapidjson文件格式json,定义：rapidjson::Document
/// msg_json;
/// @return 0:  Function::SUCCESS,
/// @return -1:  Function:FAILURE
...
/// @return 0:  Function::SUCCESS,
/// @return -1:  Function:FAILURE
int Actuator::ctrl_config_set( char* ip, uint64_t act_type, int dir, int redTat, int motor_type, int hardware_type, int vbus, int motor_dir, int pole_pairs, int max_speed, int max_acc,
                               int encoder_dir ) {
    char ctrl_config_buffer[ 1024 ] = { 0 };
    if ( fsa->ctrl_config_get( ip, NULL, ctrl_config_buffer ) != FunctionResult::SUCCESS ) {
        Logger::get_instance()->print_trace_debug( "get %s control config failed, check ip, is right?", ip );
...
/// @return 0:  Function::SUCCESS,
/// @return -1:  Function:FAILURE
int Actuator::set_encrypt( char* ip, const char* username, const char* passwold ) {
    rapidjson::StringBuffer                      set_json;
    rapidjson::Writer< rapidjson::StringBuffer > writer( set_json );
