...
int main() {
    ...
    for ( int i = 0; i < ip_num; i++ ) {
        char* ip = ( char* )ser_list[ i ].c_str();
        if ( fast_set_position_control( ip, 0.0, 0.0, 0.0, fdb_position, fdb_velocity, fdb_current ) == FunctionResult::FAILURE ) {
            Logger::get_instance()->print_trace_error( "%s set position control failed\n", ser_list[ i ].c_str() );
        }
    }
    ...
    for ( int i = 0; i < ip_num; i++ ) {
        char* ip = ( char* )ser_list[ i ].c_str();
        if ( fast_set_mode_of_operation( ip, modeOfOper.POSITION_CONTROL ) == FunctionResult::FAILURE ) {
    ...
    for ( uint32_t j = 0; j < 1000; j++ ) {
        for ( int i = 0; i < ip_num; i++ ) {
            char* ip = ( char* )ser_list[ i ].c_str();
            if ( fast_set_position_control( ip, 360 * sin( j / 10000.0 ), 0.0, 0.0, fdb_position, fdb_velocity, fdb_current ) == FunctionResult::FAILURE ) {
                Logger::get_instance()->print_trace_error( "%s set position mode failed\n", ser_list[ i ].c_str() );
            }
        }
        // usleep(1000);
