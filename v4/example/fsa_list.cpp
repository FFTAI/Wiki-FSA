...
int main() {
    ...
    if ( commandType[ "demo_comm_config_set" ].GetBool() ) {
        ...
        for ( int i = 0; i < ser_num; i++ ) {
            ...
            std::vector< int > int_array = { comm_set_msg[ "static_IP" ][ i ][ 0 ].GetInt(), comm_set_msg[ "static_IP" ][ i ][ 1 ].GetInt(), comm_set_msg[ "static_IP" ][ i ][ 2 ].GetInt(),
                                             comm_set_msg[ "static_IP" ][ i ][ 3 ].GetInt() };
            std::string        new_ip    = int_array_to_ip( int_array );

            // step6: if new_ip = broadcast_ip, then broadcast_ip will be used, else,
            // new_ip used
            /**
             * @bug it will debug question that ip was changed, cannot receive message
             * for getting config
             */
            memset( ser_msg, 0, sizeof( ser_msg ) );
    ...
    if ( commandType[ "demo_get_measured" ].GetBool() ) {
        for ( int i = 0; i < ser_num; i++ ) {
            ...
            Logger::get_instance()->print_trace_debug( "position : %f, velocity : %f, current : %f\n", msg_json[ "position" ].GetDouble(), msg_json[ "velocity" ].GetDouble(),
                                                       msg_json[ "current" ].GetDouble() );
        }
        usleep( 1000 );  // The necessary delay for running multiple scripts
    }
